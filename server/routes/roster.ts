...
rosterRouter.post("/claims", async (req, res) => {
  ...
  if (existingClaim && existingClaim !== session.userId) {
    return res.status(403).json({ error: "This character is already claimed by another user." });
  }

  const userClaim = await getClaimByDiscordId(session.userId);
  ...
  await setClaim(session.userId, characterId);
...
rosterRouter.delete("/claims/:characterId", async (req, res) => {
  ...
  if (currentClaim !== session.userId) {
...
rosterRouter.post("/farms", async (req, res) => {
  const session = await requireSession(req);
  if (!session) {
    return res.status(401).json({ error: "You must be logged in to link a farm account." });
  }
  ...
  const claimedCharacterId = await getClaimByDiscordId(session.userId);
...
rosterRouter.delete("/farms/:farmId", async (req, res) => {
  const session = await requireSession(req);
  if (!session) {
    return res.status(401).json({ error: "You must be logged in to remove a farm account." });
  }
